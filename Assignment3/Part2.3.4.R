...
#dat_ev <- dat#subset(dat, Event_ID == 1)
#t0 <- as.POSIXct(dat_ev$Timestamp[1])
#dat_ev$t <- as.numeric(difftime(as.POSIXct(dat_ev$Timestamp), t0, units = "hours"))
#dat_ev$Timestamp <- seq(0, nrow(dat_ev) - 1)
library(dplyr)

dat_ev <- dat %>%
  arrange(Event_ID, Timestamp) %>%     # ordenar por evento y tiempo
  mutate(
    t = (row_number() - 1) * (5/60)    # cada fila avanza 5 minutos = 5/60 horas
  )
...
plot(dat_ctsm$t, dat_ctsm$R, type = "l", col = "blue",  # Use common y-axis range
     ...
     main = "Observed Rainwater over time", xaxt = "n")
...
plot(dat_ctsm$t, dat_ctsm$Y, type = "l", col = "black", 
     ...
     ylab = "Stormwater", xlab = "Time (hours)", 
     main = "Observed Stormwater over time", xaxt = "n")
axis(1, at = seq(0, max(dat_ctsm$t), by = 2), labels = seq(0, max(dat_ctsm$t), by = 2))
...
plot(pred_1k$states$t.j, pred_1k[["observations"]][["Y"]], type = "l", col = "red", lty = 2, 
     ...
     ylab = "Stormwater", xlab = "Time (hours)", 
     main = "Predicted Stormwater over time", xaxt = "n")
axis(1, at = seq(0, max(dat_ctsm$t), by = 2), labels = seq(0, max(dat_ctsm$t), by = 2))
